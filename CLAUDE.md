...
- **Code blocks are never expanded.** The splitter passes fenced blocks (including indented fences, and nested fences per CommonMark fence-length rules) through byte-identical — `test/run.sh` asserts this.
